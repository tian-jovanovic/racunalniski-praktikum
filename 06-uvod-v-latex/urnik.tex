...
\subsection{Omejitve} %podrazdelek
...
\subsubsection{Stroge omejitve} % podpodrazdelek
...
\subsubsection{Šibke omejitve} % podpodrazdelek
...
\subsection{Merjenje kakovosti} %podrazdelek
...
\subsection{Požrešna metoda} %podrazdelek
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   \begin{enumerate}
      ...
      \item Za vse proste termine $\text{predavalnica}_j$ izračunamo ceno vstavljanja
      $\text{srečanje}_i$, ki ga sestavlja $\text{predmet}_i$, profesor, ki ta predmet predava,
...
\subsection{Optimizacijske metode} %podrazdelek
...
\subsubsection{Lokalno vzpenjanje} % podpodrazdelek
...
\subsubsection{Iskanje s tabuji} % podpodrazdelek
...
spremembo (npr. zamenjava $ \text{srečanje}_i $ in $ \text{srečanje}_j$ ) sedaj shranimo v
...
\subsection{Končni algoritem} %podrazdelek
...
\subsection{Zaključek} %podrazdelek
